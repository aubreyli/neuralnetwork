...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % Now, back propagate. Compute the delta error for the output layer
  output_delta = class_output - data.targets;
  % Compute the gradient for the output layer, with weight decay
  ret.hid_to_class = (1 / m) .* (output_delta * hid_output') + wd_coefficient .* model.hid_to_class;

  % Compute the delta error for the hidden layer.
  hid_delta = (model.hid_to_class' * output_delta) .* hid_output .* (1 - hid_output);
  % Compute the gradient for the hidden layer, with the weight decay.
  ret.input_to_hid = (1 / m) .* (hid_delta * data.inputs') + wd_coefficient .* model.input_to_hid;
end
